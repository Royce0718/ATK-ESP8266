...
char *ssid1 = "温一壶月光如酒";
char *password1 = "WiFi2108059136";

char *ssid2 = "Redmi K50 Pro";
char *password2 = "abcdefgh";
...
uint8_t WiFi_Connect_CUR(char *ssid, char *password)
{
    ...
    // 连接WiFi时，等待的时间要久一点
    if (Esp8266_SendAT((uint8_t *)p, (uint8_t *)"OK", 1500))
    {
        return 1; // 连接成功
    }
    else
...
void WiFi_Reconnect(uint16_t interval_second, uint16_t repeat_count)
{
    ...
    Esp8266_SendAT((uint8_t*)p, "OK", 100);
}
...
void WiFi_SetAT_CWLAP(uint16_t print_mask, int8_t rssi_filter, uint16_t authmode_mask)
{
    ...
    Esp8266_SendAT((uint8_t*)p, "OK", 1000);
}
...
void ESP_Init(void)
{
    ...
    else if (state == AP_Disconnected)
    {
        // 如果未连接WiFi,则临时连接备用AP
        printf("开始连接备用WiFi\r\n");
        if (WiFi_Connect_CUR(ssid2, password2))
        {
            printf("备用WiFi连接成功\r\n");
        }
        else
    ...
    if (WiFi_Connect_CUR(ssid1, password1))
    {
        printf("WiFi连接成功\r\n");
    }
    else
...
void Esp_GetWeather(void)
{
    ...
    HAL_Delay(500);
    AT_CIPMODE_1;
    Esp_ClientSend((uint8_t *)("GET https://api.seniverse.com/v3/weather/now.json?key=S9FlAFVDBwRs2nk6F&location=chengdu&language=en&unit=c\n\n"));
