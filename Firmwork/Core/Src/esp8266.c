...
WeatherInfo RealWeather = {0, 0, 0};

// ESP_Mode Esp_Mode = Esp_Server; // 工作在TCP Server 下
...
WiFi_Info_Node WiFi_Node1;
WiFi_Info_Node WiFi_Node2;
WiFi_Info_Node WiFi_Node3;
...
void WiFi_Init(void)
{
    ...
    // 初始化WiFi列表
    WiFiList_Init(&WiFiList);

    WiFi_Node1.WiFi_Item.ssid = "温一壶月光如酒";
    WiFi_Node1.WiFi_Item.password = "WiFi2108059136";
    WiFi_GetRssi_of(&WiFi_Node1);

    WiFi_Node2.WiFi_Item.ssid = "Redmi K50 Pro";
    WiFi_Node2.WiFi_Item.password = "abcdefgh";
    WiFi_GetRssi_of(&WiFi_Node2);

    WiFi_Node3.WiFi_Item.ssid = "你好，世界";
    WiFi_Node3.WiFi_Item.password = "Zxf187181";
    WiFi_GetRssi_of(&WiFi_Node3);

    // 将WiFi结点升序插入到WiFi列表中
    WiFiList_Insert(&WiFiList, &WiFi_Node1);
    WiFiList_Insert(&WiFiList, &WiFi_Node2);
    WiFiList_Insert(&WiFiList, &WiFi_Node3);
    ESP_Init();
}
...
void Esp_Config(void)
{
    ...
    // WiFi_Reconnect(0, 0);              // 关闭断开AP不自动连接(默认值)
}
...
void ESP_Init(void)
{
    ...
    else if (Esp_Mode == Esp_YuanZiYun)
    {
        /* 初始化连接 原子云服务器 */
        if (Esp_Connect_YuanZiYun(YuanZiYun_ID, YuanZiYun_PWD) != ESP_OK)
        {
            printf("ESP连接原子云服务器失败\r\n");
        }
        else
            printf("ESP连接原子云服务器成功\r\n");
        AT_CIPMUX_0;
    }
    else if (Esp_Mode == Esp_Client)
    {
...
void WeatherProcess(WeatherInfo *w)
{
    ...
    *p_end = 0;
    ...
    p = (uint8_t *)strstr((const char *)Uart1_RxBuffer, "code");
    p = p + 7;
    p_end = (uint8_t *)strstr((const char *)p, "\"");
    width = p_end - p;
    if (width == 1)
    {
        w->code = (WeatherCode)(*p - '0');
    }
    else if (width == 2)
    {
        w->code = (WeatherCode)((*p - '0') * 10 - (*(p + 1) - '0'));
    }

    printf("climat:%s code:%d\r\n", w->text, w->code);
...
void Esp_GetTime(void)
{
    ...
    if (Esp_Mode == Esp_Server)
    {
        /* 如果工作在TCP Server下,则转回TCP Server */
        Esp_ClientToServer();
    }
    else if (Esp_Mode == Esp_YuanZiYun)
    {
        /* 如果工作在原子云服务器连接下,则重新连接服务器 */
        AT_CIPQUIT;
        HAL_Delay(1000);
        AT_CIPCLOSE;
        Esp_Connect_YuanZiYun(YuanZiYun_ID, YuanZiYun_PWD);
    }
    else if (Esp_Mode == Esp_Client)
    {
        AT_CIPQUIT;
...
void Esp_GetWeather(void)
{
    ...
    HAL_Delay(500);
    AT_CIPMODE_1;
    Esp_ClientSend((uint8_t *)("GET https://api.seniverse.com/v3/weather/now.json?key=SkOzA0UGuEReItDhZ&location=beijing&language=en&unit=c\n\n"));
 ...
 * @param       id : 原子云设备编号
 *              pwd: 原子云设备密码
 * @retval      ESP_OK      : 连接原子云服务器成功
 *              ESP_Error   : 连接原子云服务器失败
 *              ESP_TimeOut : 连接原子云服务器超时
 */
ESP_State Esp_Connect_YuanZiYun(const char *id, const char *pwd)
{
    char p[64];
    sprintf(p, "AT+ATKCLDSTA=\"%s\",\"%s\"\r\n", id, pwd);
    return Esp8266_SendAT(p, "CLOUD CONNECTED", 1000);
}

/*****************************************************
 * @brief       ATK-MW8266D连接原子云服务器
 *
 * @retval      ESP_OK      : 断开原子云服务器成功
 *              ESP_Error   : 断开原子云服务器失败
 *              ESP_TimeOut : 断开原子云服务器超时
 */
ESP_State Esp_Disconnect_YuanZiYun(void)
{
    return Esp8266_SendAT("AT+ATKCLDCLS\r\n", "CLOUD DISCONNECT", 100);
}
